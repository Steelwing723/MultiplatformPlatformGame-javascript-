...
class Controller implements ActionListener, MouseListener, KeyListener
{
	...
	{
		if(addTubesEditor)							//For adding tubes. Hit 't' to enable
			model.addTube(e.getX()+view.scrollPosition, e.getY());
		if(addGoombasEditor)						//For adding goombas. Hit 'g' to enable
	...
	public void keyReleased(KeyEvent e)
	{
		...
		if(c == 'q') //quick exit game
				System.exit(0);
		
		if(c == 't')	//enables tube placement
		{
		...
		if(c == 'g')	//enables goomba placement
		{
	...
	void update()
	{
		Mario.yShadow = Mario.yFetch;		//gets mario's previous y position
		...
		if(keyControl)						//on control, shoot fireballs. This is on key release,
		{									//but rapid presses of the key still works, as described in instructions.
			model.addFireball();			//make a new fireball
			Fireball.fire();				//make fireball sound
			keyControl = false;				//stops fireballs from spewing out of mario's face
		}
	}
}
